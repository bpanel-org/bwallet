...
async function hardwareSignatures(
  ...
  switch (hwApp.type) {
    case 'ledger':
      {
        hwApp.logger.debug('signing using ledger');

        for (const [i, input] of mtx.inputs.entries()) {
          hwApp.logger.debug('input number: %s', i);
          ...
          hwApp.logger.debug('using segwit %s', segwit);
    ...
    case 'trezor':
      {
        hwApp.logger.debug('signing using trezor');
        ...
        for (const [i, input] of mtx.inputs.entries()) {
          ...
          hwApp.logger.debug('using segwit %s', segwit.toString());
          ...
          if (n !== -1) {
            hwApp.logger.debug('multisig detected');
          ...
          hwApp.logger.debug('using script_type %s', ti.script_type);
          inputs.push(ti);
        }
        for (const [i, output] of mtx.outputs.entries()) {
          hwApp.logger.debug('output number: %s', i);
          let address;
          ...
          hwApp.logger.debug('output address: %s', address);
          hwApp.logger.debug('output amount: %s', amount);

          // possible bug with output script_type?
          outputs.push({
            address,
            amount,
  ...
  hwApp.logger.debug('using network %s', network);
