  ...
  async _getTransactionSignatures(tx, view, inputs, outputs) {
    switch (this.type) {
      ...
      case 'trezor': {
        ...
        this.logger.debug('Using trezor signing with coin %s', coin);
        ...
        if (!result.success) throw new Error(result.payload.error);
				...
				this.logger.debug('signatures: %s', signatures.join(' '));

				const SIGHASH_ALL = Buffer.from('01', 'hex');
				this.logger.debug('appending SIGHASH_ALL 0x%s', SIGHASH_ALL.toString('hex'));

				return signatures.map(s => Buffer.concat([Buffer.from(s, 'hex'), SIGHASH_ALL]));
      }
