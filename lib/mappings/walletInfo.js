...
function mapStateToProps(state, otherProps) {
  ...
  if (accountInfo && selectedAccount in accountInfo) {
    ...
    selectedAccountAvgUTXOSize = Math.floor(selectedAccountInfo.balance.confirmed / selectedAccountUTXOCount);
    // in case divide by 0
    if (Number.isNaN(selectedAccountAvgUTXOSize)) selectedAccountAvgUTXOSize = 0;
    // parse into currency amount with label
    selectedAccountAvgUTXOSize = new Currency(chain, selectedAccountAvgUTXOSize).withLabel('unit');
  ...
  if (selectedWalletInfo) {
    ...
    selectedWalletBalance = new Currency(
      chain,
      balance,
    ).withLabel('unit');
