...
function mapStateToProps(state, otherProps) {
  ...
  // select the wallet wallet info based on the
  // selected wallet
  try {
    if (walletType === 'multisig')
      selectedWalletInfo = multisigWalletInfo[selectedWallet];
    else selectedWalletInfo = walletInfo[selectedWallet];
  } catch (e) {}
  ...
  let proposals = selectProposals(state)[selectedWallet] || [];
  let proposalDropdown = toProposalDropdown(proposals);

  // parse the selected proposal info json
  let proposalInfo = {};
  try {
    proposalInfo = proposals.find(p => p.id === selectedProposal) || {};
  } catch (e) {}
  ...
  // ui expects a string
  if (selectedProposal != null) selectedProposal = selectedProposal.toString();
...
function mapDispatchToProps(dispatch) {
  return {
    ...
    createProposal: (walletId, options) =>
      dispatch(createProposal(walletId, options)),
