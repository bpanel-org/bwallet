...
const { TAB_STORE_NAMESPACE } = INTERFACE_NAMESPACES;
...
function selectInterfaceNamespace(state, key) {
  const namespace = selectAppNamespace(state, REDUCE_INTERFACE_NAMESPACE);
  if (key) return namespace[key];
  return namespace;
}

function selectTabsNamespace(state) {
  return selectInterfaceNamespace(state, TAB_STORE_NAMESPACE);
}

/*
 * ideally, this is what this function would
 * do so that type checking doesn't have to happen
 * on the otherside
 *
 * if key in namespace
 *   return namespace[key]
 *  return new typeof namespace[key];
 */
export function selectSelectedTab(state, key) {
  const namespace = selectTabsNamespace(state);
  if (key) return namespace[key];
  return namespace;
}
...
export function selectProposals(state, key) {
  const namespace = selectWalletsNamespace(state, MULTISIG_PROPOSALS_NAMESPACE);
  if (key) return namespace[key];
  return namespace;
}

export function selectProposalMTX(state, key) {
  const namespace = selectWalletsNamespace(state, MULTISIG_PROPOSAL_NAMESPACE);
  if (key) return namespace[key];
  return namespace;
}
...
// bpanel global information
export function selectApplication(state) {
  return state.app;
}
