import { getWallets, getWalletInfo, getAccountHistory, selectWallet, selectAccount } from '../actions';
...
function buildWalletsList(wallets, multisigWallets, chain) {
  ...
  for (const msWallet of Object.values(multisigWallets)) {
    ...
    walletsList.push({
      Name: msWallet.id || '',
      Balance: new Currency(chain, balance).withLabel(
        'unit'
      ),
...
function mapDispatchToProps(dispatch) {
  return {
    ...
    selectWallet: (walletId, type, options) => dispatch(selectWallet(walletId, type, options)),
    selectAccount: (walletId, accountId) => dispatch(selectAccount(walletId, accountId)),
  };
}
