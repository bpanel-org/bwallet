...
// something undefined here, a list
const initialState = {
  [WALLETS_NAMESPACE]: [],
  [MULTISIG_WALLETS_NAMESPACE]: [],
  [WALLET_INFO_NAMESPACE]: {},
...
export default function reduceWallets(state = initialState, action) {
  ...
  logger.debug(`[wallets] type: ${type}`);
  ...
  switch (type) {
    ...
    case SET_MULTISIG_WALLET_INFO:
      return safeSet(newState, `${MULTISIG_WALLET_INFO_NAMESPACE}.${walletId}`, info);
    ...
    case SET_MULTISIG_PROPOSAL_MTX:
      return safeSet(newState, `${MULTISIG_PROPOSAL_NAMESPACE}.${walletId}.${proposalId}`, mtx);

    default:
      logger.debug(`[wallets] WARNING: default case hit, type: ${type}`);
      return state;
  }
}
