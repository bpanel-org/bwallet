...
class ReceiveFunds extends PureComponent {
  ...
  async new() {
    ...
    await createAddress(selectedWallet, selectedAccount, { addressType: 'receive' });
  }
  ...
  render() {
    ...
    return (
      <TerminalView>
        ...
        <Box justify="center">
          <QRCode text={address} />
          <Text>Funds sent to this address will be received in account {selectedAccount} in wallet {selectedWallet}</Text>
        </Box>
      </TerminalView>
    );
  }
