...
class ApproveReject extends PureComponent {
  ...
  renderProposalInfo() {
    ...
    let { proposalMTX, proposalInfo, proposalValue, proposalRecipient } = this.props;
    ...
    if (proposalInfo && 'id' in proposalInfo)
      style = {};
  ...
  renderSideCopy() {
    ...
    if (selectedProposal) {
      ...
      return (
        <div>
          <Text type="p">{m} of {n} signatures</Text>
          <Text type="p">Proposal to send {proposalValue} via wallet {selectedWallet}</Text>
        </div>
      );
    }
    return <div />
        ...
        <BoxGrid rowClass="" colcount={1}>
          <Label className="d-flex align-items-start" text="Select Proposal">
            <Dropdown
              ...
              placeholder={selectedProposal || 'Select...'}
            />
          </Label>
