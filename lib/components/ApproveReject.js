...
import { Header, Text, Input, Button, Dropdown, Link } from '@bpanel/bpanel-ui';
...
class ApproveReject extends PureComponent {
  ...
  componentDidMount() {
    ...
    if (
      selectedProposal &&
      !proposals.map(p => p.name).includes(selectedProposal)
    )
  ...
  selectProposal(proposalId, memo) {
  ...
  render() {
    ...
    return (
      ...
      >
        <BoxGrid rowClass="" colcount={1}>
          <Label className="d-flex align-items-start" text="Select Proposal">
            <Dropdown
              ...
              onChange={({ value, label }) => this.selectProposal(value, label)}
        ...
        <BoxGrid colcount={1}>
          <Header type="h4">Multi Signature Transfer</Header>
          {this.renderSideCopy()}
        </BoxGrid>
      </BoxGrid>
    );
