...
class SendFunds extends PureComponent {
  constructor(props) {
    ...
    this.dropdownOptions = HARDWARE_TYPES;
  }
  ...
  async send() {
    const { address, passphrase, amount, rate, selectedHardware } = this.state;
    const {
      sendTX,
      selectedWallet,
      selectedAccount,
  ...
  render() {
    ...
    const { address, amount, rate, passphrase, selectedHardware } = this.state;
    ...
    return (
      ...
      >
        <BoxGrid rowClass="d-inline" colcount={1}>
          ...
          {/* render hardware device dropdown when its watch only */}
          {selectedAccountInfo && selectedAccountInfo.watchOnly && (
        ...
        <BoxGrid rowClass="d-inline" colcount={1}>
          <Text className="m-auto">
            You are sending {this.convertCurrencyWithLabel(amount)}
            {address ? `to address ${address}` : ''} from {selectedAccount} in{' '}
            {selectedWallet} with a rate of {rate || '0'} satoshis per byte
          </Text>
        </BoxGrid>
      </BoxGrid>
    );
