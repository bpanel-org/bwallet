  ...
  render() {
    ...
    return (
      ...
      >
        ...
        <BoxGrid rowClass="d-inline" colcount={1}>
          <Text>
            Creating a payment proposal with{' '}
            {this.convertCurrencyWithLabel(value)}{' '}
            {recipient ? `to address ${recipient}` : ''} from {selectedWallet}{' '}
            with a rate of {rate || '0'} satoshis per byte
          </Text>
        </BoxGrid>
      </BoxGrid>
    );
