...
import { HARDWARE_TYPES, ADDRESS_TYPES } from '../constants/bwallet';
...
class NewMultisigWallet extends PureComponent {
  ...
  create() {
    ...
    // allow for legacy and segwit addresses
    let witness = false;
    if (addressType === 'Segwit') witness = true;
    ...
    return (
      ...
      >
        <BoxGrid rowClass="p-sm-2" colcount={1}>
          ...
          <Label style={labelStyle} text="Select Address Type">
            <Dropdown
          ...
          <Label style={labelStyle} text="Account Number">
            <Input
              type="number"
              className="col py-1"
              min="0"
