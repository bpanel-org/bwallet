...
class ImportWallet extends PureComponent {
  ...
  // TODO: properly pass along data
  async createAccount() {
    const { walletId, accountId } = this.state;
    await createHardwareWatchOnlyAccount(walletId, accountId, {
      chain: '',
      network: '',
      account: '',
      hardwareType: '',
  ...
  render() {
    const { accountId, derivationPath, advanced, walletId, selectedHardware } = this.state;
    ...
    return (
      ...
      >
        <BoxGrid rowClass="d-inline" colcount={1}>
          ...
          <Label
            style={isHidden}
            className="d-flex align-items-start d-none" text="Derivation Path"
          >
            <Input
              className="col"
              style={isHidden}
