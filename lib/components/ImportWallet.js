...
class ImportWallet extends PureComponent {
  ...
  static get propTypes() {
    return {
      ...
      getxpubCreateWatchOnlyWallet: PropTypes.func,
    };
  }
  ...
  async createWallet() {
    const { getxpubCreateWatchOnlyWallet, chain, network, history } = this.props;
    const { walletId, derivationPath, selectedHardware } = this.state;
    ...
    // TODO: be conditional based on success
    history.push('/bwallet');
  }

  render() {
    const { derivationPath, advanced, walletId, selectedHardware } = this.state;
