...
class SendReceive extends PureComponent {
  ...
  async componentDidUpdate(prevProps, prevState) {
    ...
    const equal = isEqual(wallets, prevProps.wallets)
  ...
  render() {
    ...
    const { allWallets, wallets, accounts, selectedWallet, selectedAccount } = this.props;
      ...
      render = (
        <div>
          ...
          <Dropdown
            ...
            onChange={({ label, value }) => this.select(label, value, 'account')}
          />
        <TabMenu tabs={tabs} />
        </div>
      );
    else if (temp === 'multisig')
      render = <TabMenu tabs={tabs} />;
