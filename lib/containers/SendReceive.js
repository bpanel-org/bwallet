...
class SendReceive extends PureComponent {
  ...
  receiveTab() {
    const { selectedWallet, selectedAccount, receiveAddress } = this.props;
    return {
      header: 'Receive',
      body: (
        <ReceiveFunds
  ...
  tabBuilder(props) {
    ...
    if (walletType === 'multisig') {
      ...
      if (!selectedWalletInfo.initialized)
        tabs.push(this.joinTab());
  ...
  render() {
    ...
    return (
      <BoxGrid rowClass="p-sm-2">
        ...
        {selectedWallet &&
          selectedAccount && <TabMenu selectedIndex={selectedTab} onTabClick={i => this.selectTab(i)} tabs={this.tabBuilder(this.props)} />}
      </BoxGrid>
    );
  }
}
