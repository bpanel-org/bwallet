...
import { Text, Header, Button, Dropdown, Link, Input, Modal } from '@bpanel/bpanel-ui';
...
class WalletInfo extends PureComponent {
  ...
  render() {
    ...
    const { createAccountView, showModal } = this.state;

    return (
      <div>
      <Modal show={showModal} header="Create Account" closeModal={() => this.toggle('showModal')}>
      ...
      <BoxGrid colcount={1}>
        <BoxGrid colClass="d-flex align-items-start" colcount={1}>
          ...
          <BoxGrid rowClass="pr-4" colcount={1}>
            ...
            <Button onClick={() => this.toggle('showModal')}>Create Account</Button>
          </BoxGrid>
        </BoxGrid>

        <BoxGrid colClass="d-flex align-items-start" childrenClass="p-2" colcount={1}>
          <BoxGrid rowClass="pr-4" colcount={1}>
            <Label text="Accounts">
              <Dropdown
                options={accounts}
