...
import { TerminalView, Box, Row, Label, RecentTxList, List } from '../components';
    ...
    return (
      <TerminalView>
        <Box justify="space-around">
          ...
          <Row>
            <Button onClick={() => console.log('not implemented')}>Details</Button>
            <Button onClick={() => this.navigate()}>Send/Receive</Button>
            <Button onClick={() => this.toggle('createAccountView')}>Add Account</Button>
          </Row>
          <List text="Accounts Overview" headers={ACCOUNTS_LIST_HEADERS} data={accounts} />
