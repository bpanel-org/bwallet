...
class WalletInfo extends PureComponent {
  ...
  selectWalletAndAccount() {
    ...
    if (selectedWallet && !accounts.includes(selectedAccount))
      selectAccount(selectedWallet, 'default', { details: true });
  }
  ...
  async broadcast(proposalId) {
    const { sendProposal, selectedWallet } = this.props;
    // bmultisig backend will sumbit raw tx
    await sendProposal(selectedWallet, proposalId);
  }
  ...
  renderPendingList() {
    ...
    const data = tableProposals.map(d => {
      ...
      if (d.Complete)
        d.Select = (
          <Button onClick={() => this.broadcast(d.id)}>Broadcast</Button>
        );
      else
        d.Select = (
          <Button onClick={() => this.selectProposal(d.id)}>Select</Button>
        );
      return d;
    });
    return (
