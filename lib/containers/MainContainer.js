...
class MainContainer extends PureComponent {
  constructor(props) {
    ...
    this.boundReset = this.resetSideNav.bind(this);
  }
  ...
  resetSideNav() {
    const { history, removeSideNav, walletsNavMeta } = this.props;
    // redirect to the main page
    history.push(`/${metadata.pathName}`);

    for (const metadata of walletsNavMeta) removeSideNav(metadata);
    this.updateWallets();
  }
  ...
  async componentDidMount() {
    ...
    this.client.on('set clients', this.boundReset);
  }

  componentWillUnmount() {
    this.updateSideNav(true);
    this.client.removeListener('set clients', this.boundReset);
  }
