...
function setSendTX(walletId, accountId, txid, status) {
  return {
    type: SEND_TX_STATUS,
    payload: { walletId, accountId, txid, status },
  };
}
...
function joinWallet(options) {
  return async (dispatch, getState) => {
    ...
    const { xpub, path } = await getxpub(opts);
...
function sendTX(walletId, options) {
  return async dispatch => {
    ...
    const { passphrase, rate, value, address, accountId } = options;
    ...
    let response;
    try {
      response = await wallet.send(opts);
    } catch (e) {
      return dispatch(setSendTX(walletId, accountId, txid, 'failure'));
    }
    const txid = response.hash;

    dispatch(setSendTX(walletId, accountId, txid, 'success'));
  };
}
