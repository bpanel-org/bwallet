...
function selectProposal(walletId, proposalId) {
  return async dispatch => {
    ...
    // only request if defined proposalId
    if (proposalId) {
      const opts = { paths: true, scripts: true, tx: true, coin: true };
      const mtx = await multisig.getProposalMTX(walletId, proposalId, opts);
      dispatch(setProposalMTX(walletId, proposalId, mtx));
...
function joinWallet(walletId, options) {
  return async (dispatch, getState) => {
    ...
    const { cosignerId, joinKey, hardwareType, path: derivationPath } = options;
    let { account } = options;
  ...
  return async (dispatch, getState) => {
    ...
    try {
      ...
      if (options.hardware) {
        ...
        const { xpub, hardwareType } = options;
...
function getMultisigProposals(walletId, options = {}) {
  return async dispatch => {
    ...
    const pending = await multisig.getProposals(walletId, true);
    const other = await multisig.getProposals(walletId, false);

    // TODO: is this best way to dedupe?
    const proposals = uniq([...pending, ...other], 'id');

    if (options.mtx) {
      for (const proposal of proposals)
        dispatch(getProposalMTX(walletId, proposal.id));
    }
    dispatch(setMultisigProposals(walletId, proposals));
  };
}

function createProposal(walletId, options) {
  return async dispatch => {
    const { multisig } = getClient();

    const { recipient, cosignerToken, memo } = options;
    let { rate, value } = options;
    if (typeof rate === 'string') rate = parseInt(rate, 10);
    if (typeof value === 'string') value = parseInt(value, 10);
    ...
    // for this to work
    await multisig.createProposal(walletId, {
      memo,
      rate,
...
function sendProposal(walletId, proposalId) {
  return async () => {
    const { multisig } = getClient();
    const response = await multisig.sendProposal(walletId, proposalId);
    const hash = response.hash;
    console.log(hash);
    // TODO: dispatch valid tx
  };
}
...
function approveProposal(walletId, proposalId, options) {
  return async (dispatch, getState) => {
    ...
    const { hardwareType, cosignerToken, account, path } = options;

    const { multisig, wallet } = getClient();
    ...
    // hardcode default account for now..
    const accountInfo = await wallet.getAccount(walletId, 'default');
    const pubkeys = [accountInfo.accountKey, ...accountInfo.keys];
