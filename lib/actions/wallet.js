...
import { getxpub, getHardwareSignatures, templateBIP44Account, toTX, signTransaction } from '../utilities';
...
function getWallets() {
  return async (dispatch, getState) => {
    ...
    if (ms.length) dispatch(setWallets(ms, 'multisig'));
  };
}
    ...
    try {
      ...
      if (options.hardware) {
        ...
        for (const input of tx.inputs) {
          ...
          const path = templateBIP44Account(chain, network, info.account);
          paths.push(`${path}/${info.branch}/${info.index}`);

          // fetch raw tx
          const inputTX = await node.getTX(input.prevout.hash);
          if (!inputTX)
            throw new Error('need transaction indexing turned on');

          const coin = Buffer.from(inputTX.hex, 'hex');
          coins.push(coin);
        }
        ...
        const response = await node.execute('sendrawtransaction', [transaction]);
        const txid = response;
