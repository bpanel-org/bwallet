...
import { getxpub, getHardwareSignatures } from '../utilities';
...
function createWallet(walletId, options, type) {
  return async dispatch => {
    ...
    if (type === 'multisig') {
      ...
      // TODO: this may need to come last
 ...
 * @param {Boolean} - options.proposal
...
async function broadcastTransaction(options) {
  return async dispatch => {
    ...
    try {
      ...
      dispatch(setSendTXFailure(walletId, accountId, {
        message: e.message,
      }))
    }
  }
