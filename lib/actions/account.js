 ...
 * @param {bool}   - details
 *   fetch wallet account history
 * @dispatches
 * @returns {void}
 */
function createAccount(walletId, accountId, options, details) {
  return async dispatch => {
    const client = getClient();
    ...
    if (details === true) dispatch(getAccounts(walletId));
  };
}
 ...
 * @param {string} - accountId
 * @param {bool}   - details
 *   fetches account history if details are set to true
 * @dispatches
 * @returns {void}
 */
function selectAccount(walletId, accountId, options = {}) {
  return async dispatch => {
    const client = getClient();

    // multisig.getAccount returns 404, normal wallet
    // getAccount returns info for both types of accounts
    const accountInfo = await client.wallet.getAccount(walletId, accountId);
...
async function getxpubCreateWatchOnly(walletId, accountId, options) {
  return async function action(dispatch, getState) {
    ...
    if (!options.chain) options.chain = selectCurrentChain(state);
    if (!options.network) options.network = selectNetwork(state);
